...
describe('Recipe Runner: resolveRecipeById', () => {
  ...
  it('builds canonical workflow/standalone discovery roots deterministically', () => {
    ...
    const userRegistry = join(userHome, '.guardrail', 'recipes');
    mkdirSync(project, { recursive: true });
    mkdirSync(join(project, 'recipes'), { recursive: true });
    mkdirSync(override, { recursive: true });
    mkdirSync(global, { recursive: true });
    mkdirSync(sharedDefaults, { recursive: true });
    mkdirSync(userRegistry, { recursive: true });

    const roots = withHomeDir(userHome, () => buildRecipeSearchDirs({
    ...
    assert.equal(roots[4], resolve(userRegistry));
  });
  ...
  it('includes configured repo and user recipe roots after local defaults', () => {
    ...
    mkdirSync(join(userHome, '.guardrail', 'recipes'), { recursive: true });
    ...
    assert.equal(roots[5], resolve(userHome, '.guardrail', 'recipes'));
  });
