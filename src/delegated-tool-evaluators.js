...
function validateAllowedPaths(paths, allowedPaths, label) {
  for (const path of paths) {
    if (!pathIsRelativeSafe(path)) return decision(false, `Path "${path}" is not a safe repo-relative path.`);
  }
  ...
  for (const path of paths) {
    ...
    if (!allowed) return decision(false, `Path "${path}" is outside delegated ${label} paths.`);
  }
...
export function evaluateRecipe(config, args) {
  ...
  if (typeof recipe !== 'string' || !recipe) return decision(false, 'recipe is required.');
  const recipes = config.recipes;
  let recipeConfig = null;
  if (Array.isArray(recipes)) {
    if (!recipes.includes(recipe)) return decision(false, `Recipe "${recipe}" is not delegated.`);
    recipeConfig = {};
  } else if (isObject(recipes)) {
    if (!(recipe in recipes)) return decision(false, `Recipe "${recipe}" is not delegated.`);
    recipeConfig = recipes[recipe] === true ? {} : recipes[recipe];
  } else {
    return decision(false, 'Tool grant must declare delegated recipes.');
  }
  const recipeHash = recipeConfig?.recipe_hash ?? recipeConfig?.recipeHash;
  if (typeof recipeHash !== 'string' || !/^[a-f0-9]{64}$/i.test(recipeHash)) {
    return decision(false, `Recipe "${recipe}" must be pinned by recipe_hash in the delegated grant.`);
  }
  ...
  if ((args.allow_unverified === true || args.allowUnverified === true) && !allowUnverified) {
    return decision(false, `Unverified recipe execution is not delegated for recipe "${recipe}".`);
  }
  ...
  for (const [key, value] of Object.entries(inputs)) {
    if (!(key in constraints)) return decision(false, `Input "${key}" is not delegated for recipe "${recipe}".`);
    if (!matchesValue(value, constraints[key])) {
      return decision(false, `Input "${key}" is outside delegated constraints for recipe "${recipe}".`);
    }
  }
...
export function evaluateHttp(config, args) {
  ...
  try {
    ...
    return decision(false, 'url must be an absolute URL.');
  }
  if (!['http:', 'https:'].includes(url.protocol)) return decision(false, 'Only http and https URLs are supported.');
  if (!LOOPBACK_HOSTS.has(url.hostname) && config.allow_remote_hosts !== true && config.allowRemoteHosts !== true) {
    return decision(false, 'HTTP requests are limited to loopback hosts unless allow_remote_hosts is explicitly true.');
  }

  const hosts = toArray(config.hosts);
  if (hosts.length > 0 && !hosts.includes(url.hostname)) return decision(false, `Host "${url.hostname}" is not delegated.`);
  const method = String(args.method || 'GET').toUpperCase();
  const methods = toArray(config.methods).map((entry) => String(entry).toUpperCase());
  if (methods.length > 0 && !methods.includes(method)) return decision(false, `Method "${method}" is not delegated.`);
  ...
  if (ports.length > 0 && !ports.includes(port)) return decision(false, `Port "${port}" is not delegated.`);
  ...
  if (Buffer.byteLength(body) > maxBodyBytes) return decision(false, `HTTP request body exceeds ${maxBodyBytes} bytes.`);
  ...
  if (!Number.isFinite(requestedTimeout) || requestedTimeout <= 0) return decision(false, 'timeout_ms must be a positive number.');
  if (requestedTimeout > maxTimeoutMs) return decision(false, `timeout_ms exceeds delegated limit ${maxTimeoutMs}.`);
  ...
  if (headers.length > 0 && allowedHeaders.length === 0) return decision(false, 'Custom headers are not delegated.');
  for (const header of headers) {
    if (!allowedHeaders.includes(header.toLowerCase())) return decision(false, `Header "${header}" is not delegated.`);
  }
...
export function evaluateService(config, grant, args) {
  ...
  if (typeof serviceId !== 'string' || !serviceId) return decision(false, 'service_id is required.');
  const serviceList = toArray(config.services);
  const declared = isObject(grant.services) && isObject(grant.services[serviceId]);
  if (serviceList.length > 0 && !serviceList.includes(serviceId)) return decision(false, `Service "${serviceId}" is not delegated.`);
  if (!declared) return decision(false, `Service "${serviceId}" is not declared by the grant.`);
  return decision(true, 'allowed');
}
...
export function evaluateGitCommit(config, args) {
  ...
  if (typeof recipeHash !== 'string' || !/^[a-f0-9]{64}$/i.test(recipeHash)) {
    return decision(false, 'guardrail_git_commit must be pinned by recipe_hash in the delegated grant.');
  }
  const paths = Array.isArray(args.paths) ? args.paths : [];
  if (paths.length === 0) return decision(false, 'paths must include at least one repo-relative path.');
  ...
  if (!pathIsRelativeSafe(messageFile)) return decision(false, 'message_file must be a safe repo-relative path.');
  if ((args.allow_unverified === true || args.allowUnverified === true) && !configAllowsUnverified(config)) {
    return decision(false, 'Unverified git commit recipe execution is not delegated.');
  }
...
export function evaluateGitDiff(config, args) {
  ...
  if (!Number.isFinite(requestedMax) || requestedMax < 0) return decision(false, 'max_bytes must be a non-negative number.');
  if (requestedMax > delegatedMax) return decision(false, `max_bytes exceeds delegated limit ${delegatedMax}.`);
...
export function evaluateGitPush(config, args) {
  const recipeHash = config.recipe_hash ?? config.recipeHash;
  if (typeof recipeHash !== 'string' || !/^[a-f0-9]{64}$/i.test(recipeHash)) {
    return decision(false, 'guardrail_git_push_feature_branch must be pinned by recipe_hash in the delegated grant.');
  }
  const remote = args.remote ?? 'origin';
  const allowedRemote = config.remote ?? 'origin';
  if (remote !== allowedRemote) return decision(false, `Remote "${remote}" is not delegated.`);
  const branch = args.branch;
  if (typeof branch !== 'string' || !branch) return decision(false, 'branch is required.');
  const pattern = config.branch_pattern ?? config.branchPattern ?? '^(feature|bugfix|chore|docs|refactor|test|ci)/[A-Za-z0-9._/-]{1,96}$';
  if (!new RegExp(pattern).test(branch)) return decision(false, `Branch "${branch}" is outside delegated branch pattern.`);
  if (/^(main|master|production|prod|staging|release\/.+)$/.test(branch)) return decision(false, `Branch "${branch}" is protected.`);
  if ((args.allow_unverified === true || args.allowUnverified === true) && !configAllowsUnverified(config)) {
    return decision(false, 'Unverified git push recipe execution is not delegated.');
  }
