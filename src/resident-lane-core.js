...
import {
  cleanupLaneArtifacts as cleanupLaneArtifactsRuntime,
  createLaneBootError,
  launchResidentLaneDaemonHelper as launchResidentLaneDaemonHelperRuntime,
  persistLaneFailureState as persistLaneFailureStateRuntime,
  runResidentLaneDaemon as runResidentLaneDaemonRuntime,
  waitForResidentLaneBootstrap as waitForResidentLaneBootstrapRuntime,
...
export { createLaneBootError } from './lane/runtime.js';
...
export function persistLaneFailureState(options, err, failureStage = 'bootstrap') {
  return persistLaneFailureStateRuntime(options, err, failureStage, {
    buildState,
    lanePaths,
    removeLaneClaim,
    writeJson,
...
export async function waitForResidentLaneBootstrap(options, child, deps = {}) {
  return waitForResidentLaneBootstrapRuntime(options, child, {
    isPidAlive,
    lanePaths,
    readJson,
    readLogTail,
    ...
    postStartGraceMs: POST_START_GRACE_MS,
    ...deps,
  });
}
...
function cleanupLaneArtifacts(options, status, extra = {}) {
  return cleanupLaneArtifactsRuntime(options, status, extra, {
    buildState,
    lanePaths,
    removeHostLaneRegistryEntry,
    removeLaneClaim,
...
export async function runResidentLaneDaemon(options, adapter, deps = {}) {
  return runResidentLaneDaemonRuntime(options, adapter, {
    appendFileSync,
    buildState,
    cleanupLaneArtifacts,
    defaultHealthTimeoutMs: DEFAULT_HEALTH_TIMEOUT_MS,
    ensureLaneLayout,
    lanePaths,
    laneResultPath,
    maxRequestBytes: MAX_REQUEST_BYTES,
    partialRequestTimeoutMs: PARTIAL_REQUEST_TIMEOUT_MS,
    readLaneControl,
    readSecretFromFd,
...
export function launchResidentLaneDaemonHelper(options, adapter) {
  return launchResidentLaneDaemonHelperRuntime(options, adapter, {
    ensureLaneLayout,
    lanePaths,
  });
}
