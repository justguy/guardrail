...
The remaining Claude-specific gap is now narrower and explicitly documented: the resident interactive wrapper can detect readiness, paste into Claude's TUI, and submit a tiny direct turn successfully on the host runtime using the kitty keyboard Enter sequence. The current blocker has moved from submit/auth/transport to completion detection for longer file-pointer turns: small prompts execute, but longer resident-lane packet turns still need stronger turn-complete detection so Guardrail does not misclassify wrapper-level completion as packet completion.
