...
Templates enforce structured mode only (no shell), require constrained inputs (pattern or enum for strings), and use an explicit environment handshake so templates cannot silently harvest env vars. Runtime child commands execute from the caller's working directory: for CLI runs this is the directory where `guardrail run --template` is invoked, and for delegated MCP runs this is the delegated `repo_path`. Templates stored under `.guardrail/templates` can safely use repo-relative paths such as `artifacts/...` without creating symlinks back into the repo.
...
Keep an interactive AI session alive behind one bounded lane, then send later user messages without reopening the outer transport every turn. This is the primary Guardrail path for multi-shot AI work, fire trials, human review loops, and any run where the operator may need to ask questions or give feedback mid-stream. `claude-exec` is now a deprecated compatibility surface for narrow one-shot probes only. For autonomous write-bearing packet work, choose the Claude permission mode deliberately: `--permission-mode acceptEdits` is suitable when edits should proceed without constant approval prompts, but a fully autonomous packet that must also execute local proof commands can still stall there. In that case use `--permission-mode dontAsk` and explicitly expose the tool set Claude needs, for example `--allowed-tools "Bash Read Edit Write Glob Grep"`, inside the already-bounded Guardrail lane scope:
...
# Or keep Guardrail waiting on the same request/result path
guardrail lane send \
  --id claude-live \
...
# Read status, latest result, and bounded logs in one command
guardrail lane inspect --id claude-live --tail 40

# Read the latest or named result without reopening the outer transport
guardrail lane result --id claude-live

# Wait on a long-running request without dropping to raw host inspection
...
# Permanently revoke a lane so it cannot be restarted (writes REVOKED sentinel + audits lane_revoked)
guardrail lane revoke --lane-dir .guardrail/lanes/claude-live --actor ops --reason "policy breach"
...
# Filter that portfolio view before taking action
guardrail lane list --status failed --tool-filter codex --has-conflicts --json
...
guardrail lane start \
  ...
  --tool local-exec \
  --command python3 \
  --arg scripts/prompt-consumer.py \
  --session-name grep-live

# Start a generic prompt-wrapper lane for a local wrapper that accepts
# --prompt / --working-dir / --session-name / --lifecycle
guardrail lane start \
  --id wrapper-live \
  --tool prompt-wrapper \
  --wrapper-command ./scripts/my-wrapper.js \
  --wrapper-arg mode=review \
  --session-name wrapper-live

# Start a remote prompt-wrapper lane over SSH
guardrail lane start \
  --id remote-wrapper-live \
  --tool ssh-prompt-wrapper \
  --ssh-target build@staging.example.com \
  --remote-working-dir /srv/app \
  --wrapper-command guarded-wrapper \
  --wrapper-arg mode=review \
  --session-name remote-wrapper-live
...
Resident lanes are for direct interactive use when the executable boundary should stay fixed but later user messages should not count as execution drift. The shipped lane control plane now supports `claude`, `codex`, `local-exec`, `prompt-wrapper`, and `ssh-prompt-wrapper` adapters behind the same `lane start` / `lane send` / `lane result` / `lane wait` / `lane status` / `lane inspect` / `lane history` / `lane cleanup` / `lane stop` surface, and `lane adapters` exposes the bundled adapter inventory plus adapter capabilities. Unknown `--tool` values now fail closed instead of silently falling back. The current helper:
- creates owner-only FIFOs (`0600`) under the lane directory
- stores an ephemeral per-lane HMAC key under a repo-local host root (`.guardrail/host-lanes/...`) by default, so normal lane start/send/status/inspect traffic stays inside the Guardrail repo boundary
...
- writes a repo-local live-lane registry entry by default and can still widen to host-level mirrored views when `lane list --all-repos` or lane startup coordination needs broader visibility
- appends `lane_start`, `lane_send`, `lane_result`, `lane_wait`, `lane_cleanup`, and `lane_stop` lifecycle entries to `.guardrail/audit.jsonl` so later ops review can distinguish lane creation, message traffic, bounded polling, targeted cleanup, result reads, expiry, and explicit teardown

Resident lanes are also the first step toward the broader "manager of managers" direction for Guardrail. Today Guardrail ships lifecycle control for individual named lanes, repo-local and host-registry lane discovery, tool-selectable local and remote adapters, startup locking, swarm scope coordination, typed non-path resource claims, and bounded audit/history surfaces. Guardrail now also ships `lane portfolio` as the bounded swarm-timeline surface: it can read the current repo lane audit or the mirrored host-level lane audit (`--all-repos`), summarize event/tool/status counts, show the currently visible live-lane snapshot beside that timeline, and preserve prune/cleanup tombstone paths so operators can review startup, cleanup, prune, and conflict-related lane events without stitching together per-lane history by hand.

Lane startup still has to happen in a runtime where the downstream CLI auth already works. Direct AI recipes can now declare bounded `requires_auth` checks too, so Guardrail fails before launch with `missing_auth_prerequisite` instead of letting the underlying CLI die late. The same bounded preflight now applies when those recipes are executed through workflow `recipe_ref` steps, so chained recipe workflows stop before launch on missing tool auth instead of surfacing a late downstream CLI failure. `lane start` now launches the daemon through a short-lived helper so the resident lane survives the wrapped CLI process exiting, records a fresh boot nonce in the lane identity record, uses repo-local `.guardrail/host-lanes/...` key and registry paths by default for autonomous runs, and still uses mirrored host-level views plus startup lock files to fail closed when broader duplicate-live-lane coordination is needed. Resident Claude lanes now also run an explicit auth preflight inside the daemon runtime before the first packet is accepted: Guardrail classifies the auth source (`operator_config_dir`, `env_token`, or `keychain`), runs the smallest real Claude wrapper probe that matches lane execution, and fails early with structured `auth_preflight_failed` or `auth_probe_failed` lane state instead of consuming a packet attempt just to discover login drift. Lanes can also declare `--scope-type repo|worktree|paths` plus `--scope-mode warn|block`, repeated `--scope-path <relative-path>` values, and typed `--resource <class:name>` claims with `--resource-mode warn|block` for non-path ownership such as `git-branch:main`, `service:postgres`, or `env:staging`. Guardrail now canonicalizes legacy `branch:<name>` to `git-branch:<name>`, auto-discovers the current `git-branch:<name>` claim when the working tree is on an attached branch and no explicit branch claim was provided, and compares repo-scoped branch ownership separately from host-scoped resources like services or environments. When callers narrow `working_dir` below the repo root and omit explicit scope flags, Guardrail now infers a default `worktree/warn` ownership scope from that working directory. Guardrail compares those declared or inferred scopes and resource claims against other live lanes, blocks startup on overlapping `block` claims, and surfaces overlapping ownership through `lane start`, `lane status`, and `lane list`. Later `lane send` turns reuse that resident lane instead of launching a fresh outer transport hop each time. If the lane has expired, `lane send` returns a structured `lane_expired` error and the correct recovery is to run `lane start` again. Use `--tool claude` (default), `--tool codex`, `--tool local-exec`, `--tool prompt-wrapper`, or `--tool ssh-prompt-wrapper` to bind that lane to the right wrapped executor.
Guardrail now exposes first-class startup and long-turn state for resident lanes. If a request outlives the client-side wait window, `lane send` returns a structured `pending` response with the request id and an exact bounded next step instead of collapsing into `lane_expired`. `lane start` also now fails early with `lane_boot_failed` when the daemon dies during bootstrap or in the immediate post-start window, and `lane status` reports `failureReason`, `failureStage`, `logPath`, transport summary, and a concrete next command when a lane is in `failed` state. Claude-backed lanes additionally persist `authSource`, `authPreflightStatus`, `authPreflightReason`, `authPreflightMessage`, and `authPreflightCheckedAt`, so an auth failure is visible as lane state before any packet is sent. If the daemon disappears before the first request and leaves no explicit failure metadata, Guardrail now infers that as `failed/post_start` instead of showing a bare `stale` lane. Use `lane status` to see whether the lane is `ready`, `busy`, `failed`, `expired`, `stale`, or `stopped`, including the current request id/start time, lane identity metadata, transport summary, and the last completed result path. Use `lane inspect` when you want one bounded surface that combines status, latest result, bounded logs, and recent audit history with chain-validity. Use `lane history` when you want just the resident-lane audit timeline. Use `lane portfolio` when you want the repo-level or host-level lane timeline instead of one lane’s history. Use `lane result` to read the stored output for the latest or named request once it completes, and `lane wait` when you want Guardrail to wait on that same result instead of dropping to raw host inspection. Use `lane run-sequence` when you already know you need multiple prompt files to run sequentially on the same live lane and want Guardrail to stay attached through long-running steps until each one completes or fails; add `--stop-when-done` when the lane should be torn down after the final successful step instead of being left open to idle-expire. Use `lane list` to inspect every repo-local lane in one portfolio view, add `--all-repos` when you need to include host-registry lanes from other checkouts, narrow that view first with `--status`, `--tool-filter`, `--lane-id-filter`, `--session-name-filter`, `--scope-type-filter`, `--scope-mode-filter`, `--resource-filter`, `--alive`, or `--has-conflicts`, use `lane cleanup` to remove one failed/dead lane directly once diagnosis is complete, and use `lane prune` for bulk dead-lane cleanup with explicit audit entries. `lane prune --dry-run` now previews the exact candidates and reasons before deletion, and real prune runs write a bounded tombstone under `.guardrail/lane-tombstones/` before deleting the lane directory so post-mortem review survives cleanup. Raw host inspection should be the last resort, not the default recovery path.
The Claude-specific resident-lane gap `D0za` is now closed. The interactive wrapper can detect readiness, paste into Claude's TUI, submit with the proven `\r\r` path, and complete both a tiny direct turn and a bounded artifact/report turn on the host runtime. The wrapper now keeps the primary beacon-based flow when Claude renders the expected UI markers, and also has bounded fallback behavior for newer Claude TUI versions that accept paste silently without emitting the older visual cue.
...
- `lane start`: one-time host-runtime startup for a selected tool (`claude` by default; also `codex`, `local-exec`, `prompt-wrapper`, and `ssh-prompt-wrapper`)
- `scope-type` / `scope-mode` / `scope-path`: optional lane ownership declaration for concurrent agents (`repo`, `worktree`, or explicit repo-relative paths)
- `resource` / `resource-mode`: optional non-path ownership declaration for resources like `git-branch:main` or `service:postgres`
- `lane send`: later message traffic through the resident lane
- `lane send --wait`: send one message and keep Guardrail polling for completion on the same request
- `lane result`: read the stored output for the latest or named request
- `lane wait`: wait for a resident-lane request to complete through the bounded lane surface
- `lane inspect`: inspect status, latest result, and bounded logs together
- `lane history`: inspect recent lane audit entries only
- `lane portfolio`: inspect the repo-level or host-level lane timeline plus the current live-lane snapshot
...
- `lane cleanup`: remove one failed/dead lane directly once you know it should not be reused
- `lane stop`: explicit teardown
- `lane extend`: live-update a running lane's idle/health timeouts or send a bounded heartbeat (`--idle-timeout-ms`, `--health-timeout-ms`, `--heartbeat`). The daemon re-reads the lane's `control.json` every poll tick, so extension changes take effect on the running lane without restart.

Lane health vs. idle expiry:
- `healthTimeoutMs` (default `300000`, 5 min): after this much idle time the lane transitions to the soft `stalled` state. The lane is not torn down; operators can inspect it, send a heartbeat, or extend timeouts.
- `idleTimeoutMs` (default `900000`, 15 min): hard expiry. Must remain longer than `healthTimeoutMs`. Only this triggers daemon shutdown.
- Incoming real requests naturally clear `stalled`; `lane extend --heartbeat` refreshes the activity timestamp without faking a completion.
- `lane list`: enumerate repo-local lanes and their current state
- `lane list --all-repos`: include host-registry lanes from other checkouts that share the same Guardrail host state
- `lane list --status/--tool-filter/--lane-id-filter/--session-name-filter/--scope-type-filter/--scope-mode-filter/--resource-filter/--alive/--has-conflicts`: narrow that portfolio view before cleanup or restart decisions (`--resource-filter git-branch` matches any branch claim; `--resource-filter git-branch:main` stays exact)
- `lane adapters`: inspect bundled lane adapters before startup
- `lane prune`: classify and remove dead lane artifacts from the repo-local registry
- `lane prune --dry-run`: preview prune candidates and reasons without deleting anything
- `lane portfolio --all-repos`: inspect the mirrored host-level lane timeline across repos
...
- if explicit scope flags are omitted and `working_dir` narrows below the repo root, Guardrail infers `scope-type=worktree` with `warn`
...
- `resource-mode=warn|block`: same enforcement model for explicit non-path claims
- Conflict comparison now includes repo-local state plus the host-side resident-lane registry when duplicate live lanes are checked across checkouts
...
### Policy Simulation (Pre-Rollout Impact Preview)

Before committing a new policy or rolling out a change to existing rules, operators can ask what the policy engine would decide for a given execution context — without triggering any execution, acquiring any lock, or producing any approval record.

```bash
# Ask what would happen if a developer ran terraform apply under the current policy
...
# Same query with machine-readable JSON output (exit 0 = allow, exit 1 = deny)
guardrail policy simulate \
...
The simulation result carries:
- `decision` — `allow` or `deny`
- `simulated: true` — guaranteed marker so dry-run output is never confused with real authorization
- `risk_level` and `trust_class` — same values the supervisor would compute at execution time
- `matched_rules` — structured array identifying each risk-engine rule and policy rule that drove the decision (source: `risk_engine`, `local_policy`, or `org_policy`)
- `trace.checks` — ordered list of checks that ran, each with `name`, `result`, and `detail`
- `reasons` — human-readable list of risk signals for the decision

The simulation path calls the same `evaluateRisk()` and `enforcePolicy()` functions as real execution — no separate code path, no divergence risk.
...
The bundled `codex-exec` and `claude-exec` recipes are Guardrail-managed wrappers around `codex exec` and `claude --print`. `codex-exec` remains a normal bounded wrapper. `claude-exec` is deprecated and kept only as a compatibility surface while resident lanes are the primary Guardrail-managed Claude path. They support:
...
- For long-running AI packets, also pass `--input report_artifact=<relative/path.md>` when you know the canonical report file up front. Guardrail passes that path into the wrapper contract and treats report-file mtime changes as synthetic progress heartbeats.
...
- Guardrail now mitigates that slightly when `report_artifact` is declared: report-file mtime changes become synthetic `ai_artifact_written` progress events even if Claude never writes NDJSON checkpoints itself.
- If you need true back-and-forth review, feedback, and question-answering during a run, prefer the resident FIFO lane. The one-shot progress channel is for bounded visibility and continuation, not for replacing the lane model.
...
- MCP profiles are recognized and may declare a validated `mcp_transport` contract, and Guardrail now supports bounded MCP discovery plus bounded `call` / `batch` runtime paths over declared `stdio` transport
- shell-style MCP execution is still intentionally blocked, and blocked MCP runs tell you which transport contract was recognized so users can distinguish “profile is malformed” from “transport exists on paper but is not live yet”
...
- **Protocol limits**: Phase 1 supports `stdin-json`, `env-shim`, and bounded `mcp` execution over declared stdio transports; arbitrary shell-style MCP runs remain blocked
...
guardrail adapter profile discover aider --json
...
- `adapter probe` is discovery only. It does not run user commands through MCP profiles. Use it to confirm that the declared stdio transport can initialize and expose tools under Guardrail.
...
- `adapter run` for MCP profiles now accepts only bounded structured requests. Use `--mcp-tool` with `--params-json` for one explicit MCP call or `--calls-json` for an explicit ordered batch. Shell-style `adapter run -- <command>` remains intentionally blocked for MCP profiles.
- Signed adapter-profile index verification is now shipped via `guardrail adapter profile index verify <path> --index-key <pubkey.pem>`, and Guardrail can now also resolve `adapter profile install <tool-name>` through that verified local index when you pass both `--index` and `--index-key` or configure `trusted_adapter_indexes` in `~/.guardrail/config.json`.
- `adapter profile discover [tool-name] [--json]` lists the tools currently exposed by those configured trusted signed indexes before you install one.
- Bare-name installs through trusted signed indexes now fail closed on ambiguity and record the chosen index path plus signer `key_id` in the installed pin metadata. This is still a local/team distribution flow, not ambient unsigned public discovery.
...
- For actual multi-shot operator-in-the-loop execution, do not keep reusing one-shot `claude-exec`. Start a resident lane and use `lane chat` / `lane send` / `lane wait` / `lane result` as the primary interface. Treat `claude-exec` as deprecated compatibility-only unless you are forced into a one-shot probe or legacy flow.
...
A grant does not have to be a broad global file. Prefer per-repo/per-agent grants, for example `~/.guardrail/mcp-grants/<repo-slug>-codex.json`, when one project needs different tools, hashes, env allow-lists, or service declarations. Agents should not edit a shared file such as `codex-global.json` to approve their own work. If a run is outside the active grant, use MCP host elicitation or the CLI approval-request fallback when available; otherwise ask the operator to issue or update an operator-controlled grant and restart the MCP server with that grant path.
...
| **Policy scenarios** (30+) | Declarative policy → expected decision (GREEN/YELLOW/RED, channel, strict mode, simulation) |
