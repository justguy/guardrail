...
The remaining Claude-specific gap is now narrower and explicitly documented: the resident interactive wrapper can detect readiness, paste into Claude's TUI, submit using the kitty keyboard Enter sequence, and complete a tiny direct turn successfully on the host runtime. The current blocker has moved from submit/auth/transport to longer packet-style prompt execution: short interactive turns are now trustworthy, but longer resident-lane packet/file-pointer turns still do not complete reliably enough to call the interactive Claude backend fully closed.
