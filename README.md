...
The remaining Claude-specific gap is now much narrower and explicitly documented: the resident interactive wrapper can detect readiness and paste the prompt into Claude's TUI, but the live host-runtime probes still show no deterministic submit binding after paste. Paste-only, paste plus single `\r`, and paste plus single `\n` all stop at Claude's `[Pasted text …]` acknowledgement with no processing marker. That means the current blocker is Claude interactive submit semantics, not lane transport, auth preflight, or prompt delivery.
