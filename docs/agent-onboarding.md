...
- Template execution cwd is the caller's runtime cwd, not the template file directory. For CLI mode, run `guardrail run --template` from the intended repo/workspace. For delegated MCP mode, child commands run from the delegated `repo_path`. Do not create repo-to-template symlinks just to make relative paths such as `artifacts/...` resolve.
...
- Do not assume the generic 60000ms worker fallback is enough for AI CLI recipes. `claude-exec` now carries its own bounded 900000ms / 15m invoke timeout because packet-sized Claude runs can legitimately exceed one minute.
...
- For long-running bounded implementation packets, require the model to create the declared report artifact immediately and append short progress checkpoints there. `claude --print` is still a one-shot CLI surface, but Guardrail now also ships a first-class progress side channel in `.guardrail/ai-progress.ndjson` plus `.guardrail/ai-progress-state.json`.
- When you know the report path up front, pass it explicitly with `report_artifact`. Guardrail now feeds that path into the wrapper contract and treats report-file mtime changes as synthetic progress heartbeats even if the model fails to emit NDJSON checkpoints itself.
- Use `guardrail recipe progress --state-dir .guardrail --follow` as the primary live-monitoring surface for one-shot Claude packets. It surfaces the Guardrail-owned progress stream plus soft states such as `waiting_for_review`, `waiting_for_input`, `drift_warning`, and `stalled` without dropping to raw host inspection.
- If the packet lands in a continuation-eligible soft state, resume it through `guardrail recipe continue --state-dir .guardrail --prompt "<response>"`. That preserves the same bounded session identity instead of forcing a brand-new run.
- Treat the report file as the human-readable heartbeat and proof artifact, but do not treat a stale report alone as terminal failure if the Guardrail-owned progress channel or bounded file diffs are still moving.
- For true interactive review loops, packet-by-packet fire trials, and any execution where the operator may need to redirect or question the worker mid-run, use the resident FIFO lane as the default path. The one-shot `claude-exec` progress channel improves observability, but `claude-exec` is now deprecated and remains only a compatibility fallback rather than a primary execution model.
- For autonomous write-bearing Claude packets, prefer `mode=acceptEdits` instead of `mode=default`. The default permission mode can leave a non-interactive run idling on tool-approval behavior that Guardrail cannot surface as useful live progress.
- For autonomous resident-lane Claude packets that must also run local proof commands such as `node --test`, `acceptEdits` can still leave the inner Claude run waiting on tool approval. Start the bounded lane with `--permission-mode dontAsk` and explicitly expose the needed tools with `--allowed-tools "Bash Read Edit Write Glob Grep"` so Claude can actually execute the proof path instead of only reporting status about it.
- Claude-backed lanes and bundled Claude recipes now default the LLM budget to `$10.00` unless the operator overrides it, and Guardrail review/approval output shows that budget explicitly. Do not hide or silently change that budget in fire-trial or production-facing operator flows.
- Guardrail governs Guardrail approvals, the bounded execution contract, and Guardrail-native monitoring. It does not govern Claude's own internal tool policy. If Claude itself surfaces a tool-approval or tool-availability issue, treat that as downstream CLI behavior owned by the operator/user, not as Guardrail silently taking control of Claude.
...
- If the supervising environment already approved the `node src/cli.js lane` command prefix, reuse that approved prefix directly for later `lane send`, `lane status`, `lane inspect`, `lane wait`, and `lane result` calls. Do not keep re-requesting escalation for routine lane monitoring or continuation unless the boundary itself changes.
...
- `lane send` is the per-message step. It reads the repo-local lane key through the Guardrail CLI by default, signs the request, writes the strict JSON payload into the lane FIFO, and reads the matching response back without reopening the outer transport/runtime hop.
...
- If you already know the next run needs multiple prompt files on the same lane, use `lane run-sequence` instead of hand-driving repeated `send` / `wait` calls. `lane run-sequence` now stays attached through long-running steps until each step resolves, and `--stop-when-done` closes the lane after the final successful step when you do not want to leave it open.
...
- The repo-local lane key is ephemeral by default. If the daemon idles out, crashes, or is stopped, the key is deleted. Later `lane send` calls should treat missing key/FIFO state as `lane_expired` and start a fresh lane instead of trying to resume stale secrets.
...
- `claude-exec` now preserves the wider current runtime env during the bounded Claude probe/invoke path so secure-store-backed auth matches the real shell/runtime more closely. The declared `--env-allow` subset is still approval-bound and still the part that Guardrail records for drift; the broader inherited runtime env is there only to preserve Claude exec parity with the shell you launched Guardrail from.
- For bundled `claude-exec`, use the recipe path only for compatibility, narrow probes, or legacy flows where a one-shot bounded wrapper is still unavoidable. In current shipped recipe mode, `prompt` is session-bound `interactive_message` while `system_prompt` remains `review_each_time`. That means later user messages can change inside the same persistent named session, but a changed `system_prompt` or a new session still re-prompts. For normal Guardrail-managed Claude work, use resident lanes instead.
...
- For Guardrail's delegated MCP server, the template omnitool is the parent `guardrail_template` tool with `action: "describe"`, `"prepare"`, `"request_approval"`, or `"run"`. It routes to the existing template supervisor and approval model; legacy `guardrail_template_*` and `guardrail_run_template` tools are compatibility aliases, not separate approval paths. If `guardrail_template` appears under `toolInventory.exposedButNotGrantedTools`, ask for an updated operator-controlled grant instead of falling back to raw shell or a hidden tool name.
- MCP grant files must be operator-controlled and outside the delegated repo, MCP server cwd, and Git worktree root. Repo-local grant files and repo-local symlinks are rejected because a repo-writing agent could approve itself. A grant can still be project-specific: use a per-repo/per-agent file under `~/.guardrail/mcp-grants/` and start the MCP server with that path rather than editing a broad shared/global grant during agent work.
