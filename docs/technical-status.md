...
| D0za | Claude interactive submit/complete contract | v0.5 | Deterministic resident-lane execution on the interactive Claude backend without falling back to one-shot `--print` | In progress — the resident interactive wrapper now reaches the real Claude TUI, detects readiness, pastes atomically, and can execute a tiny direct turn on the host runtime using the kitty keyboard Enter sequence (`ESC[13u]`). PTY probes and send/receive logs in `docs/plans/REPORT_claude_tui_paste_submit_semantics.md`, `docs/plans/REPORT_claude_tui_submit_diagnosis_from_send_receive_logs.md`, and `docs/plans/REPORT_sonnet_direct_lane_submit_blocker.md` narrowed the remaining gap again: submit for short turns works, but longer file-pointer packet turns still need stronger completion detection so Guardrail does not treat wrapper-level completion as packet completion. |
