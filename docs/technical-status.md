...
| D0t | Lane registry and portfolio visibility (`lane list`, `lane prune`) | v0.4 | First-class "manager of managers" UX for concurrent lanes | Done (partial) — Guardrail now ships `lane list`, targeted `lane cleanup`, and `lane prune` over the repo-local `.guardrail/lanes/*` registry, can optionally widen visibility with `lane list --all-repos` through the host-side live-lane registry, surfaces mixed ready/busy/failed/expired/stale/stopped lane state in one view, supports first-class list filters for status/tool/lane-id/session-name/scope/resource/conflict state, and appends explicit `lane_cleanup` / `lane_prune` audit entries when dead lanes are removed. `lane prune` now classifies candidates before deleting them, supports `--dry-run`, records prune reasons/signals in the returned result, and writes repo-local lane tombstones under `.guardrail/lane-tombstones/` before deleting dead lane directories so post-mortem review survives cleanup. Current prune semantics still intentionally target dead lanes only (`stale`, `expired`, `stopped` by default; `failed` is opt-in) and broader portfolio UX such as batch lifecycle actions remains open. |
...
| `lane portfolio [--all-repos] [--limit <n>]` | Done | Query the repo-level or mirrored host-level resident-lane timeline in one bounded surface |
...
| `lane prune` | Done (partial) | Classify dead lane candidates, support `--dry-run`, write lane tombstones, and remove dead lane artifacts from the repo-local lane registry with audit entries |
