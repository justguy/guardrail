...
| D0w | Transport-generalized resident lanes | v0.5 | Extend the lane control plane beyond the first Claude proof | Not started — lift lane lifecycle/state/result handling out of the Claude-specific implementation into a transport-agnostic lane contract, then adapt bundled transport recipes and adapter flows incrementally. Keep the promise centered on reusable bounded productivity lanes, not on sandboxing or enterprise identity features. |
