...
| A0h | Additive MCP transport for adapter mode | v0.3 | Native MCP-tool integration without regressing current adapters | Partial — MCP profiles now declare a validated `mcp_transport` contract (currently `stdio` only) with explicit correlation/capability-discovery expectations, and blocked runtime paths surface the recognized transport in their diagnostics. Guardrail now also ships `adapter probe` as a bounded stdio discovery path: it runs the declared MCP transport under approval, performs only `initialize` + `tools/list`, and reports discovered tools without enabling general MCP command execution. `adapter run` for MCP profiles remains intentionally blocked until approval, drift, trust, and auth-preflight semantics are implemented end to end. |
