...
| Bundled recipes | Done | npm-publish, git-branch-cleanup, git-commit, github-pr-merge, dep-upgrade, infra-deploy, openclaw-wrapper, openclaw-fix-tests, codex-exec, claude-exec, cmux-claude-exec |
...
| R0f | Task-specific OpenClaw recipes (`fix-tests`, `debug-ci`, `deploy`) | v0.3 | Safer task-oriented agent entrypoints on top of the existing wrapper | Done (partial) — Guardrail now ships the first bounded task-specific recipe, `openclaw-fix-tests`, which fixes both the flow id and scope while preserving the wrapper’s scope check and output verification steps. Additional task-specific flows (`debug-ci`, `deploy`) should follow the same pattern instead of introducing broader unconstrained wrappers. |
