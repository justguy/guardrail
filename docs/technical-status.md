...
| D0q | Transport + exec recipe composition under one approval | v0.4 | Common host-runtime hops without nested double-approval UX | Done (partial) — Guardrail now supports composed transport+exec recipe steps under one supervisor-managed approval contract, with child recipe trust/env/input-hash/session metadata bound into the outer manifest and no nested inner `guardrail run`. The bundled `cmux-claude-exec` recipe is the first shipped proof and now includes one bounded hosted auth-repair attempt (`claude auth login --console` in the exact target runtime, then probe + retry once) before surfacing `auth_repair_pending_user_input` or the final hosted failure. Broader artifact/CLI ergonomics remain open; see `docs/plans/PLAN_transport_exec_recipe_composition.md`. |
| D0r | Resident transport sessions and approval handoff | v0.4 | Approve a bounded host-runtime lane once, then trigger repeated bounded runs without repeated surface-level approval | Done (partial) — Guardrail now ships a FIFO-based resident lane control plane with `lane start` / `lane send` / `lane result` / `lane wait` / `lane status` / `lane inspect` / `lane logs` / `lane cleanup` / `lane stop` CLI support. Lane startup creates owner-only request/response FIFOs, generates an ephemeral repo-scoped host key outside the workspace, and keeps the authenticated host-runtime session alive for later prompts. Follow-up sends use the FIFO bridge instead of reopening the outer launcher/surface hop, `lane send` returns a structured `pending` result when the request is still running and now points bounded recovery at `lane wait`/`lane inspect`, `lane wait` provides a bounded wait/read path for that same request without dropping to raw host inspection, `lane inspect` combines status, latest result, and bounded logs in one surface, `lane start` now fails early with `lane_boot_failed` when the daemon dies during bootstrap or the immediate post-start window, `lane status` and `lane list` now emit exact next-command guidance, `lane logs` exposes a bounded local log tail for diagnosis, `lane result` reads stored output artifacts without reopening the transport, missing key/FIFO state yields a structured `lane_expired` recovery path, duplicate signed request ids are rejected inside the active lane window, and lane lifecycle actions now append `lane_start` / `lane_send` / `lane_result` / `lane_wait` / `lane_cleanup` / `lane_stop` events to `.guardrail/audit.jsonl`. Broader transport coverage and richer audit/ops UX remain open; see `docs/plans/PLAN_resident_transport_sessions_and_approval_handoff.md`. |
| D0s | Interactive user-message sessions without per-message reapproval | v0.4 | Direct chat with a guarded AI runtime where the user controls message content but not the executable boundary | Done (partial) — recipe/template approval envelopes now support `interactive_message`, and Guardrail now also ships a resident FIFO lane path so later user prompts can be sent into the same approved persistent AI session without turning each message into executable drift or reopening the host-runtime transport. Runtime-boundary changes, session-identity changes, and `review_each_time` companions like `system_prompt` still force reapproval. Review-loop ergonomics are materially better now that long-running turns surface as explicit pending/result state instead of ambiguous lane failure, `lane send --wait` can keep the operator inside Guardrail-managed polling, and text-mode lane surfaces now preserve request/result identity instead of dropping that context. Broader transport-resident UX remains open; see `docs/plans/PLAN_interactive_user_message_sessions.md`. |
| D0t | Lane registry and portfolio visibility (`lane list`, `lane prune`) | v0.4 | First-class "manager of managers" UX for concurrent lanes | Done (partial) — Guardrail now ships `lane list`, targeted `lane cleanup`, and `lane prune` over the repo-local `.guardrail/lanes/*` registry, surfaces mixed ready/busy/failed/expired/stale/stopped lane state in one view, supports first-class list filters for status/tool/lane-id/session-name/scope/conflict state, and appends explicit `lane_cleanup` / `lane_prune` audit entries when dead lanes are removed. Current prune semantics intentionally target dead lanes only (`stale`, `expired`, `stopped` by default; `failed` is opt-in) and broader portfolio UX such as batch lifecycle actions remains open. |
| D0u | Stable lane identity and split-brain prevention | v0.4 | Honest lane ownership across restarts, retries, and multi-project use | Done (partial) — resident lanes now persist an explicit `identity.json` record plus fresh boot nonces, `lane start` fails closed if another live lane with the same lane id already exists in the same repo lane registry, and the default host key path is now scoped by stable Guardrail repo identity instead of the bare lane id. Reuse-vs-restart decisions no longer rely only on leftover FIFOs or a global key filename. Startup lock files and broader multi-project duplicate-live-lane protection are still open. |
| D0v | Swarm scope coordination for concurrent lanes | v0.5 | Multi-agent productivity without accidental workspace collisions | Done (partial) — resident lanes can now declare optional `repo`, `worktree`, or explicit `paths` scopes plus `warn|block` enforcement at startup, and Guardrail now infers a default `worktree/warn` scope when `working_dir` narrows below the repo root and callers omit explicit scope flags. Guardrail persists that ownership in lane identity/state, compares it only against other live lanes in the same repo registry, blocks startup on overlapping `block` scopes, and surfaces overlapping live-lane conflicts through `lane start`, `lane status`, and filtered `lane list` views. Broader resource classes and cross-repo coordination remain open. |
| D0w | Transport-generalized resident lanes | v0.5 | Extend the lane control plane beyond the first Claude proof | Done (partial) — the resident lane CLI surface is now explicitly tool-selectable (`--tool claude|codex`), unknown tool ids fail closed instead of silently falling back, `lane adapters` exposes bundled adapter inventory plus adapter capability metadata sourced from the adapter modules themselves, lane identity/state/list/status all persist the selected tool, the generic `src/resident-lane.js` entrypoint now owns the shared lifecycle exports directly from the lane core, and the same FIFO lane control plane drives both the Claude and Codex exec wrappers through tool-specific adapters. The lane contract is still adapter-based rather than fully transport-plugin-driven, and bundled transport recipes/adapters beyond these first AI wrappers remain open. |
...
| # | Feature | Target | Unlocks | Status |
|---|---------|--------|---------|--------|
| H0a | Harden human-domain routing gates and risk scoring for benchmark/evaluation use | v0.4 | Safe reruns of human/workplace benchmark subsets without leaking sensitive content or silently misrouting CT | Done (partial) — Guardrail now ships reusable helper modules for this work: balanced-object LLM JSON extraction plus redacted parser-failure metadata in `src/llm-json.js`, and runtime normalization/recomputation helpers for domain routing, premise rejection, and human-risk scores in `src/human-domain-routing.js`, with focused regression coverage. The actual benchmark-runner wiring and replacement of the external sketch’s `parseLLMJson()` / gate callers are still outside this repo and remain open in `docs/plans/PLAN_human_domain_routing_gates_and_risk_scoring.md`. |
...
| `lane start --id <id> [--tool claude|codex]` | Done (partial) | Start a resident interactive lane, create FIFOs, mint the ephemeral host key, and bind the lane to a selected tool adapter |
...
| `lane list` | Done (partial) | Enumerate repo-local resident lanes and narrow the portfolio with status/tool/scope/conflict filters |
| `lane prune` | Done (partial) | Remove dead lane artifacts from the repo-local lane registry with audit entries |
