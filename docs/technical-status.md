...
| D0r | Resident transport sessions and approval handoff | v0.4 | Approve a bounded host-runtime lane once, then trigger repeated bounded runs without repeated surface-level approval | Done (partial) — Guardrail now ships a FIFO-based resident lane control plane with `lane start` / `lane send` / `lane result` / `lane status` / `lane stop` CLI support. Lane startup creates owner-only request/response FIFOs, generates an ephemeral per-lane host key under `~/.guardrail/lanes/<id>.key`, and keeps the authenticated host-runtime session alive for later prompts. Follow-up sends use the FIFO bridge instead of reopening the outer launcher/surface hop, `lane send` returns a structured `pending` result when the request is still running, `lane start` now fails early with `lane_boot_failed` when the daemon dies during bootstrap or the immediate post-start window, `lane status` exposes ready/busy/failed/expired/stale state plus the current request id/start time, the last completed result path, persisted startup failure details, and the selected tool, `lane result` reads stored output artifacts without reopening the transport, missing key/FIFO state yields a structured `lane_expired` recovery path, duplicate signed request ids are rejected inside the active lane window, and lane lifecycle actions now append `lane_start` / `lane_send` / `lane_result` / `lane_stop` events to `.guardrail/audit.jsonl`. Broader transport coverage and richer audit/ops UX remain open; see `docs/plans/PLAN_resident_transport_sessions_and_approval_handoff.md`. |
| D0s | Interactive user-message sessions without per-message reapproval | v0.4 | Direct chat with a guarded AI runtime where the user controls message content but not the executable boundary | Done (partial) — recipe/template approval envelopes now support `interactive_message`, and Guardrail now also ships a resident FIFO lane path so later user prompts can be sent into the same approved persistent AI session without turning each message into executable drift or reopening the host-runtime transport. Runtime-boundary changes, session-identity changes, and `review_each_time` companions like `system_prompt` still force reapproval. Review-loop ergonomics are materially better now that long-running turns surface as explicit pending/result state instead of ambiguous lane failure, but Guardrail session reports still need real proof validation against the branch state. Broader transport-resident UX remains open; see `docs/plans/PLAN_interactive_user_message_sessions.md`. |
| D0t | Lane registry and portfolio visibility (`lane list`, `lane prune`) | v0.4 | First-class "manager of managers" UX for concurrent lanes | Done (partial) — Guardrail now ships `lane list` and `lane prune` over the repo-local `.guardrail/lanes/*` registry, surfaces mixed ready/busy/failed/expired/stale/stopped lane state in one view, and appends explicit `lane_prune` audit entries when dead lanes are removed. Current prune semantics intentionally target dead lanes only (`stale`, `expired`, `stopped` by default; `failed` is opt-in) and broader portfolio UX such as batch actions or richer filters remains open. |
| D0u | Stable lane identity and split-brain prevention | v0.4 | Honest lane ownership across restarts, retries, and multi-project use | Done (partial) — resident lanes now persist an explicit `identity.json` record plus fresh boot nonces, and `lane start` fails closed if another live lane with the same lane id already exists in the same repo lane registry. Reuse-vs-restart decisions no longer rely only on leftover FIFOs or a global key filename. Full repo-scoped host-key isolation, startup lock files, and broader multi-project duplicate-live-lane protection are still open. |
| D0v | Swarm scope coordination for concurrent lanes | v0.5 | Multi-agent productivity without accidental workspace collisions | Done (partial) — resident lanes can now declare optional `repo`, `worktree`, or explicit `paths` scopes plus `warn|block` enforcement at startup. Guardrail persists that ownership in lane identity/state, compares it only against other live lanes in the same repo registry, blocks startup on overlapping `block` scopes, and surfaces overlapping live-lane conflicts through `lane start`, `lane status`, and `lane list`. Broader resource classes, automatic scope inference, and cross-repo coordination remain open. |
| D0w | Transport-generalized resident lanes | v0.5 | Extend the lane control plane beyond the first Claude proof | Done (partial) — the resident lane CLI surface is now explicitly tool-selectable (`--tool claude|codex`), lane identity/state/list/status all persist the selected tool, the generic `src/resident-lane.js` entrypoint fronts the lane lifecycle API, and the same FIFO lane control plane now drives both the Claude and Codex exec wrappers through tool-specific adapters. The lane contract is still adapter-based rather than fully transport-plugin-driven, and bundled transport recipes/adapters beyond these first AI wrappers remain open. |
...
| D2 | Self-hosted recipe registry (JSON API) | v0.5 | Enterprise air-gap | Partial — `guardrail recipe registry export <output-dir>` now writes a static self-hosted registry snapshot under `v1/recipes/` with top-level inventory, per-recipe metadata, and per-version JSON documents suitable for static hosting. Remote registry discovery/install and `trusted_registries` enforcement are still open. |

### Recipe Expansion Candidates (v0.3–v0.4)

| # | Feature | Target | Unlocks | Status |
|---|---------|--------|---------|--------|
| R0a | Ship-now bounded recipe batch (`gh-open-pr`, `git-clone-allowed`, `gh-release`, `docker-build`, `docker-push`, `terraform-plan-only`) | v0.3 | Safe expansion of high-signal day-one recipes | Partial — the first shipped slice is `terraform-plan-only`, a structured validate+plan-only Terraform recipe with bounded relative config paths and no mutation or saved-plan output. The broader recipe batch remains open; see `docs/plans/PLAN_recipe_launch_batch_ship_now_2026-04-09.md`. |
...
| A0h | Additive MCP transport for adapter mode | v0.3 | Native MCP-tool integration without regressing current adapters | Partial — MCP profiles now declare a validated `mcp_transport` contract (currently `stdio` only) with explicit correlation/capability-discovery expectations, and blocked runtime paths surface the recognized transport in their diagnostics. Guardrail now ships four bounded MCP surfaces under approval: `adapter probe` performs only `initialize` + `tools/list`, `adapter mcp tools` exposes the discovered MCP tool metadata as an explicit inventory view, `adapter mcp call` performs exactly one `tools/call` with an explicit MCP tool name and JSON params, and `adapter mcp batch` performs an explicit ordered array of `{ tool, params }` calls over one approved stdio session. When capability discovery is required, both bounded call surfaces validate the requested MCP tool set against the discovered MCP inventory before launching the runtime transport. `adapter run` for MCP profiles remains intentionally blocked until broader approval, drift, trust, and auth-preflight semantics are implemented end to end for general MCP execution. |
| A1 | Signed index + bare-name profile install | v0.3 | Discovery and easier onboarding | Partial — Guardrail now ships signed adapter-profile index verification through `adapter profile index verify <path> --index-key <pubkey.pem>`, including schema validation plus signature verification over SHA-pinned `github://...@<40-char-sha>` entries. `adapter profile install <tool-name>` can now resolve through that verified local signed index when the caller explicitly supplies `--index <path>` and `--index-key <pubkey.pem>`. Ambient public bare-name discovery remains intentionally blocked until trusted-index verification exists for a public distribution channel. |
...
| `lane list` | Done | Enumerate repo-local resident lanes and their current state |
