...
| D0d | Unified workflow recipe discovery (installed/home/global parity, no manual `--recipe-search-dir`) | v0.3 | Cross-repo recipe workflows without local glue | Not started — current workflow mode supports explicit `--recipe-search-dir` for immediate portability. Long term, workflow `recipe_ref` resolution should share the same installed/home/global recipe discovery model as standalone recipe mode. |
| D0e | Bundled wrapper portability (remove manual `guardrail_repo` plumbing for Guardrail-shipped recipes) | v0.3 | Easier cross-repo agent workflows | Not started — bundled recipes like `claude-exec` and `git-commit` still require a repo path to locate wrapper helpers. Long term, Guardrail-shipped wrappers should be discoverable/packageable without passing the checkout path as an execution input. |
...
| D0r | Resident transport sessions and approval handoff | v0.4 | Approve a bounded host-runtime lane once, then trigger repeated bounded runs without repeated surface-level approval | Not started — today every transport invocation is a fresh execution boundary even when the launcher/runtime lane is unchanged. Add a resident transport/session model that can keep an approved host-runtime lane alive, accept only bounded composed-exec triggers, and re-check trigger drift honestly without forcing a fresh outer approval each time. Safe shipping requires explicit lane identity, trigger schema binding, TTL/idle expiry, replay protection, teardown controls, per-trigger audit records, and clear separation between “lane creation approval” and later “bounded trigger reuse.” |
| D0s | Interactive user-message sessions without per-message reapproval | v0.4 | Direct chat with a guarded AI runtime where the user controls message content but not the executable boundary | Not started — today recipe-mode `prompt` is part of the approval surface, which is honest for automation but too blunt for direct user interaction. Add a first-class interactive session mode where runtime/tool/model/cwd/env/budget/transport are approved once, user messages flow inside that fixed lane without reapproving each turn, and reapproval happens only when the executable boundary, session identity, or trust/env policy changes. |
...
| A0g | Agent session contracts and resumable named runs | v0.3 | Repeatable long-lived AI workflows | Done — `claude-exec` and `codex-exec` recipes accept `lifecycle` (`start` / `continue` / `attach`), `session_name`, and `session_id` inputs. Recipe supervisor enforces bounded session contracts at `<projectRoot>/.guardrail/agent-sessions/<recipeId>/<slot>.json` with `session_missing`, `session_drift`, `session_attach_mismatch`, and `session_already_exists` fail-closed reasons. Session enforcement runs BEFORE manifest reuse so drift is detected even when the manifest would otherwise match. Prompt inputs keep `review_each_time` reapproval semantics; the session contract never short-circuits prompt reapproval. Wrappers emit structured `[guardrail-session]` metadata to stderr before spawning the underlying CLI — no scraping of `~/.claude/*` or `~/.codex/*`, no implicit attachment to ambient local sessions. |
| A0h | Additive MCP transport for adapter mode | v0.3 | Native MCP-tool integration without regressing current adapters | Not started — introduce MCP as an explicit opt-in transport layer for MCP-first tools such as Cline while keeping `stdin-json` and `env-shim` intact. It must preserve the same approval, drift, trust, and auth-preflight semantics, fail closed on transport/correlation errors, and never silently replace the existing adapter paths. |
