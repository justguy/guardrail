...
- 26 focused tests in `test-human-domain-routing.js` + 4 wiring tests in `test-adapter-runtime.js`. All 107 focused-proof tests pass (0 failures).
...
| D0za | Claude interactive submit contract | v0.5 | Deterministic resident-lane execution on the interactive Claude backend without falling back to one-shot `--print` | In progress — the resident interactive wrapper now reaches the real Claude TUI, detects readiness, and pastes the packet into the input buffer on the host runtime. PTY probes recorded in `docs/plans/REPORT_claude_tui_paste_submit_semantics.md` prove that paste-only, paste plus single `\r`, and paste plus single `\n` all stop at Claude's `[Pasted text …]` acknowledgement without starting processing. The remaining gap is therefore the post-paste submit model on the interactive Claude backend, not lane transport, auth preflight, repo-local state, or prompt delivery. |
