...
| D0x | Portfolio-wide lane audit and net-intent timeline | v0.5 | Explain what the swarm owns, changed, and cleaned up across the whole host | Planned — Guardrail now has per-lane `lane history`, repo-local/host-wide lane registries, prune reasons, and tombstones, but it still lacks one portfolio-level timeline that explains ownership changes, conflict decisions, startup blocks, and prune/cleanup outcomes across all lanes in one bounded surface. |
