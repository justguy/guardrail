...
| D0za | Claude interactive submit/complete contract | v0.5 | Deterministic resident-lane execution on the interactive Claude backend without falling back to one-shot `--print` | In progress — the resident interactive wrapper now reaches the real Claude TUI, detects readiness, pastes atomically, submits with the kitty keyboard Enter sequence (`ESC[13u]`), and can complete a tiny direct turn successfully on the host runtime. Guardrail now also threads explicit `reportArtifact` / `completionMode` metadata through resident-lane requests so direct turns and artifact-driven packet turns can use different completion rules, and wrapper-side Tcl/expect failures now fail closed instead of being reported as success. PTY probes and send/receive logs in `docs/plans/REPORT_claude_tui_paste_submit_semantics.md`, `docs/plans/REPORT_claude_tui_submit_diagnosis_from_send_receive_logs.md`, and `docs/plans/REPORT_sonnet_direct_lane_submit_blocker.md` narrowed the remaining gap again: short direct turns are proven, but longer packet/file-pointer turns still do not complete reliably enough to call the interactive Claude backend fully closed. |
