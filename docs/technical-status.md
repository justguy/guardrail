...
| D0t | Lane registry and portfolio visibility (`lane list`, `lane prune`) | v0.4 | First-class "manager of managers" UX for concurrent lanes | Not started — add project-local lane discovery over `.guardrail/lanes/*/state.json`, surface active/busy/failed/expired/stale/stopped lanes in one view, and implement `lane prune` to clean stale artifacts with explicit audit entries. This is a productivity and operability feature, not a host-isolation feature. |
| D0u | Stable lane identity and split-brain prevention | v0.4 | Honest lane ownership across restarts, retries, and multi-project use | Not started — replace implicit global key-path assumptions with an explicit lane identity record, repo-scoped ownership, startup lock files or boot nonces, and fail-closed duplicate-live-lane detection. Reuse-vs-restart semantics should become explicit instead of being inferred from leftover artifacts. |
