...
| D0v | Swarm scope coordination for concurrent lanes | v0.5 | Multi-agent productivity without accidental workspace collisions | Not started — let lanes optionally declare intended write/work scopes, compare those scopes against active lanes at start/reuse time, and warn or block on conflicting write ownership. Initial shipping target should be repo/worktree/path scopes surfaced through `lane start`, `lane status`, and `lane list`, before broader resource classes. |
