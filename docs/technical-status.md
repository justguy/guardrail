...
| # | Feature | Target | Unlocks | Status |
|---|---------|--------|---------|--------|
| H0a | Harden human-domain routing gates and risk scoring for benchmark/evaluation use | v0.4 | Safe reruns of human/workplace benchmark subsets without leaking sensitive content or silently misrouting CT | Planned — Guardrail has a tracked design plan in `docs/plans/PLAN_human_domain_routing_gates_and_risk_scoring.md`. The needed work is: replace naive `{...}` extraction in `parseLLMJson()` with safer structured parsing, add runtime validation and local recomputation/clamping for routing/risk outputs, stop raw sensitive-content logging on parser failure, make the bypass policy explicit (`skipCT`-style flags renamed to policy terms), and only then rerun the benchmark subset (`W04`, `W15`, `S01`, `S13`). This is intentionally tracked as evaluation-safety hardening, not as enterprise-only work. |
