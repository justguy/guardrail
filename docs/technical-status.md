...
- The server exposes only grant-delegated tools for recipe and template describe/prepare/request-approval/run, local service lifecycle, and bounded loopback HTTP probes; git workflows are exposed as ordinary recipes instead of git-named MCP tools.
- Template workflows now have an omnitool-style parent `guardrail_template` MCP tool with explicit `action` values for `describe`, `prepare`, `request_approval`, and `run`. This is an agent-facing parent surface over the existing template supervisor, not a separate template runtime. Legacy template-specific MCP tool names remain callable compatibility aliases over the same policy and runtime paths.
- `guardrail_grant_status` now separates actual callable tools from grant-only stale declarations via `toolInventory`, so agents do not infer that unexposed entries such as old `guardrail_git_*` grants are callable MCP tools.
...
- Delegated template calls now execute child processes from the delegated `repo_path`, not from `.guardrail/templates`; CLI template runs execute from the caller's current working directory. Relative runtime paths stay repo/workspace-relative while template discovery and approved manifests remain under `.guardrail/templates`.
- Delegated recipe/template execution can be approved by pinned `recipe_hash` / `template_hash`, MCP host form elicitation for unpinned delegated runs, or the CLI `approval_request_id` fallback. Host elicitation binds the prompt to the tool, repo path, recipe/template name, hashes, inputs, env, manifest path, and risk/setup summary. Hosts without MCP elicitation fail closed with `host_approval_unavailable`; declined, cancelled, or malformed host responses also fail closed before supervisor execution.
...
- Repo containment now uses real paths to catch symlink escapes, repo-local MCP grant files/symlinks are rejected, HTTP/diff resource limits are grant-enforced, MCP tool calls are serialized per runtime, and focused coverage lives in `tests/test-mcp-policy.js` plus `tests/test-mcp-server.js`. Project-specific grants should be operator-controlled per-repo/per-agent files outside the worktree, not agent-edited broad global grants.

---

## Recent: Repo-local Claude raw-git hook (2026-04-16)

- Added a zero-dependency Claude `PreToolUse` hook in `src/claude-git-guardrail-hook.js`; operators can enable it through their own repository or user-level Claude configuration.
...
- [x] **Delegated Guardrail MCP server** (`src/mcp-server.js`, zero new deps, stdio JSON-RPC). Ships a grant-scoped execution slice for recipe describe/prepare/run, template describe/prepare/run, local service start/stop/status, and bounded HTTP probes. Grants are agent-bound, expiring, repo-contained, and must be stored outside agent-writable repo roots; execution can use hash-pinned grants, MCP host elicitation for unpinned delegated runs, or the CLI `approval_request_id` fallback. Delegated template processes run from the delegated repo path. Unsupported host elicitation fails closed with `host_approval_unavailable`, declined or malformed host responses fail closed before supervisor execution, and git workflows are exposed as ordinary recipes instead of git-named MCP tools.
