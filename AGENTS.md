...
## Git Automation Expectations

Do not require live operator approval for every Git mutation. The default operating mode should minimize operator bottlenecks while keeping destructive and remote-mutating operations bounded.

Agents may perform ordinary local Git workflow operations needed to complete an explicitly requested task, including branch creation/switching, exact-path staging, commits, and status/diff/log inspection. When committing, keep the commit scope tight to the requested work and do not stage unrelated dirty files.

Remote mutation still must not bypass the raw Git guardrails above. Use the shipped Guardrail recipes for normal push and force-push workflows instead of raw `git push`.

Stop and ask the operator only when the next Git step is destructive, ambiguous, or likely to discard work: worktree/history wipes, branch deletion, reset/restore/clean forms, force push outside the shipped safe recipe, unresolved merge/rebase conflicts, or staging files unrelated to the requested task.
...
- For git, read-only commands such as `git status`, `git diff`, and `git log` are acceptable. Ordinary local Git workflow mutations may proceed when they are scoped to the requested task. Remote-mutating or destructive raw Git commands must respect the raw Git guardrails above.
